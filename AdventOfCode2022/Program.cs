...
U 18";
            ...
            positions.Add(tailPosition.ToString(), 1);

            for (int i = 0; i < movements.Length; i++)
            {
                string[] movement = movements[i].Split(" ");
                string direction = movement[0];
                int length = int.Parse(movement[1]);
                switch (direction)
                {
                    case "U":
                        ...
                        {
                            tailPosition.Y--;
                            UpdateTailPosition(tailPosition, headPosition, positions);
                        }
                        break;
                    case "R":
                        for (int j = 0; j < length; j++)
        ...
        private static void UpdateTailPosition(Point tailPosition, Point headPosition, Dictionary<string, int> positions)
        {
            if (tailPosition.X == headPosition.X)
            {
                int distance = tailPosition.Y - headPosition.Y;
                if (Math.Abs(distance) <= 1)
                {
                    UpdatePositions(tailPosition, positions);
                }
            }
        }

        private static void UpdatePositions(Point tailPosition, Dictionary<string, int> positions)
        {
            if (positions.ContainsKey(tailPosition.ToString()))
            {
                positions[tailPosition.ToString()]++;
            }
            else
            {
                positions.Add(tailPosition.ToString(), 1);
        ...
        public static void AoC8()
        {
            bool IsSecondPart = true;

            string input = @"301220120123412411222431204050154502352345215611331265331250145342122214525502214010044420343123123
...
011201113143220302144232532235500205436316601263264003140631445266055502252414045114101420043032302";
            ...
            for (int i = 0; i < forest.GetLength(0); i++)
            {
                for (int j = 0; j < forest.GetLength(1); j++)
                {
                    ...
                    {
                        int scenicScore = CalculateScenicScore(forest, i, j);
                        if(scenicScore > visibleScore)
                        {
                            visibleScore = scenicScore;
                        }
                    }
                }
            }

            Console.WriteLine(visibleScore);
        }

        private static bool CheckVisibility(int[,] forest, int row, int column)
        {
            if (row == 0 || column == 0 || row == forest.GetLength(0) || column == forest.GetLength(0))
            {
                return true;
            }
            ...
            bool southVisible = true;
            bool westVisible = true;
            bool eastVisible = true;
            // Check North
            ...
            // Check East
            for (int i = column + 1; i < forest.GetLength(0); i++)
            {
        ...
        private static int CalculateScenicScore(int[,] forest, int row, int column)
        {
            ...
            int height = forest[row, column];
            int northViewDistance = 0;
            int southViewDistance = 0;
            int westViewDistance = 0;
            int eastViewDistance = 0;
            // Check North
            for (int i = row - 1; i >= 0; i--)
            {
                northViewDistance++;
            ...
            return northViewDistance * eastViewDistance * southViewDistance * westViewDistance;
        }
